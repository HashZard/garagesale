...
- ADR-0002 替代 ADR-0001 中关于隔离 local、staging、production 数据环境的决定；只保留一套环境，本地开发与部署共用同一套外部服务配置，不存在 preview 实例。
- `docs/prd.md` 收敛为纯产品范围文档：技术栈细节、目录结构与数据库结构不再由它持有，分别指向架构基线、`AGENTS.md` 与 `docs/data-model.md`，消除重构后长期存在的第二份技术事实来源。
